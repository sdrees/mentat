...
pub fn txs_after(conn: &TestConn, after: Entid) -> Vec<Tx> {
    let transactions = transactions_after(
        &conn.sqlite, &conn.schema, after
    ...
    for transaction in transactions.0 {
        ...
        for datom in &transaction.0 {
            ...
            let a = match datom.a {
                ...
                EntidOrIdent::Ident(ref a) => conn.schema.get_entid(a).unwrap().0,
            };
...
pub fn parts_to_datoms(schema: &Schema, parts: Vec<TxPart>) -> Datoms {
    Datoms(parts.iter().map(|p| part_to_datom(schema, p.clone())).collect())
}
