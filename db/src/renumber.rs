...
pub fn renumber(conn: &rusqlite::Connection, root: &PartitionMap, local: &PartitionMap, remote: &PartitionMap) -> Result<DB> {
        ...
        if let Some(remote_partition) = remote.get(name) {
            ...
            if let Some(root_partition) = root.get(name) {
                ...
                if advanced_locally_by > 0 && advanced_remotely_by > 0 {
                    ...
                    let tx_e = "UPDATE timelined_transactions SET e = e + ? WHERE timeline = 0 AND e >= ? AND e < ?";
    ...
    let mut db = read_db(conn)?;
    db.partition_map = output;
    Ok(db)
}
...
mod tests {
    ...
    fn test_renumber_non_trivial_schema() {
        ...
        assert_matches!(local.last_transaction(),
                        r#"[[65538 :db/ident :foo/local 268435458 true]
                            [65538 :db/valueType :db.type/long 268435458 true]
        ...
        // transactions table.  Note that 65539 = 65536 + 3 (four allocated entids),
        // and 268435459 = 268435458 + 1 (two transactions).
        assert_matches!(local.last_transaction(),
                        r#"[[65539 :db/ident :foo/local 268435459 true]
                            [65539 :db/valueType :db.type/long 268435459 true]
