...
impl Conn {
    ...
    pub fn q_once<T>(&self,
                     ...
                     query: &str,
                     inputs: T) -> TransactionResult<QueryOutput>
    ...
    pub fn q_explain<T>(&self,
                        sqlite: &rusqlite::Connection,
                        query: &str,
                        inputs: T) -> TransactionResult<QueryExplanation>
        where T: Into<Option<QueryInputs>>
    {
        let metadata = self.metadata.lock().unwrap();
        let known = Known::new(&*metadata.schema, Some(&metadata.attribute_cache));
    ...
    pub fn pull_attributes_for_entities<E, A>(&self,
                                              sqlite: &rusqlite::Connection,
                                              entities: E,
                                              attributes: A) -> TransactionResult<BTreeMap<Entid, ValueRc<StructuredMap>>>
    ...
    pub fn pull_attributes_for_entity<A>(&self,
                                         sqlite: &rusqlite::Connection,
                                         entity: Entid,
                                         attributes: A) -> TransactionResult<StructuredMap>
    ...
    pub fn lookup_values_for_attribute(&self,
                                       sqlite: &rusqlite::Connection,
                                       entity: Entid,
                                       attribute: &edn::Keyword) -> TransactionResult<Vec<TypedValue>> {
        let metadata = self.metadata.lock().unwrap();
        let known = Known::new(&*metadata.schema, Some(&metadata.attribute_cache));
    ...
    pub fn lookup_value_for_attribute(&self,
                                      sqlite: &rusqlite::Connection,
                                      entity: Entid,
                                      attribute: &edn::Keyword) -> TransactionResult<Option<TypedValue>> {
        let metadata = self.metadata.lock().unwrap();
        let known = Known::new(&*metadata.schema, Some(&metadata.attribute_cache));
...
mod tests {
    ...
    fn test_transact_does_not_collide_existing_entids() {
        ...
        match conn.transact(&mut sqlite, t.as_str()) {
            Err(MentatError::TransactionError(TransactionError::DbError(e))) => {
    ...
    fn test_transact_does_not_collide_new_entids() {
        ...
        match conn.transact(&mut sqlite, t.as_str()) {
            Err(MentatError::TransactionError(TransactionError::DbError(e))) => {
                ...
                assert_eq!(e.kind(), ::mentat_db::DbErrorKind::UnallocatedEntid(next));
            },
            x => panic!("expected db error, got {:?}", x),
        }
    ...
    fn test_transact_errors() {
        ...
        match report.expect_err("expected transact error") {
            MentatError::TransactionError(TransactionError::DbError(e)) => {
                match e.kind() {
