    ...
    where T: Into<Option<QueryInputs>>
{
    ...
    if !unbound.is_empty() {
        bail!(TransactionError::UnboundVariables(unbound.into_iter().map(|v| v.to_string()).collect()));
    }
    Ok(algebrized)
}
...
fn lookup_attribute(schema: &Schema, attribute: &Keyword) -> Result<KnownEntid> {
    schema.get_entid(attribute)
          .ok_or_else(|| TransactionError::UnknownAttribute(attribute.name().into()).into())
}
